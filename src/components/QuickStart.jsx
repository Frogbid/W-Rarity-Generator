...
function HomePage() {
    ...
    let [NFTBalances, setNFTBalances] = useState();
    let [collection, setCollection] = useState();
    let [nft, setNft] = useState();
    let {Moralis} = useMoralis();

    let handleChangeCollection = async (col) => {
    ...
    let handleSelectToken = async (num, col) => {
        if (num && col) {
            const dbNFTs = Moralis.Object.extend(col);
            const query = new Moralis.Query(dbNFTs);
            console.log(num);
    ...
    let collectionChanged = async (col) => {
        console.log("excute");
    ...
    useEffect(async () => {
        let ignore = false;
        let col = "GoldSilverPirates";
        if (!ignore) {
            await setCollection(col);
            await handleSelectToken(token, col);
            await handleChangeCollection(col);
        }
        return () => {
            ignore = true;
        }
        ...
        <>
            <div>
                ...
                >
                    ...
                    >
                        <option value="GoldSilverPirates">GoldSilverPirates</option>
                    </Select>*/}
